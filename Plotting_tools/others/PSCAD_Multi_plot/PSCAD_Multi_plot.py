...
def plot(out_folder, out_file, out_chans,outchans_adj, filename, plot_title=["NA"] * 12, plot_x_label=["Time (s)"] * 12,
    ...
    _ch_id, _ch_data, time = [], [], []
    for file in xrange(len(out_file)):
        #read PSCAD
        infname = out_file[file]
        # PSCAD output files
        import glob, re
        import os
        import numpy as np
        #
        outfiles = []
        for pscadfile in glob.glob(infname[0:-4] + "_*.OUT"):
        ...
        data1 = np.zeros((len(ch_id), len(time1)))
        #
        ...
        for i in range(len (data1)-1):
            ch_data[i+1] = data1[i+1].tolist()
        ch_data['time'] = data1[0].tolist()
        # Setup time array using data
        print('PSCAD channels: ' + str(ch_id))
        ...
        for y in xrange(len(out_file)):
            ...
            if isinstance(out_chans[0][sub], list):  
                ...
                if data[y][sub][0] != -1:
                    ...
                    if prepost == filename[0]:
                        plt.plot(time, data[y][sub][x], label=_ch_id[y][out_chans[0][sub][x]] + prepost)
    ...
    file_str = os.path.join(out_folder, out_file[0]).replace('%s.inf'%(filename[0]), '.pdf')
...
def plot_all_out_files(folder=[''], chans=None, chans_adj=None, filename =[], ylabel=[""] * 12, xlabel=["Time (s)"] * 12, title=["NA"] * 12, xlims="",
    ...
    # path = folder + "/*.out"
    print(str(glob.glob(folder + "/*" + filename[0] + ".inf")))
    print(filename)
    for filepre in glob.glob(folder + "/*" + filename[0] + ".inf"):
        print(filepre)
        for filepost in glob.glob(folder + "/*" + filename[1] + "*.inf"):
            if filepre[0:filepre.find(filename[0])] == filepost[0:filepost.find(filename[1])]:
                plot(folder, [filepre, filepost], chans,chans_adj, filename, plot_title=title, plot_x_label=xlabel, plot_y_label=ylabel,
